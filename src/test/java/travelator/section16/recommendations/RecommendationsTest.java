...
import travelator.section16.Id;
import travelator.section16.destinations.FeaturedDestination;
import travelator.section16.destinations.FeaturedDestinations;
import travelator.section16.domain.DistanceCalculator;
...
class RecommendationsTest {
    ...
    private final FeaturedDestinations featuredDestinations = mock(FeaturedDestinations.class);
    ...
    @Test
    void returns_no_recommendations_when_no_locations() {
        assertEquals(emptyList(), recommendations.recommendationsFor(emptySet()));
    }

    @Test
    void returns_no_recommendations_when_no_featured() {
        givenFeaturedDestinationsFor(paris, emptyList());
        assertEquals(emptyList(), recommendations.recommendationsFor(Set.of(paris)));
    }

    @Test
    void returns_recommendations_for_single_location() {
        givenFeaturedDestinationsFor(paris, List.of(eiffelTower, louvre));
        givenADistanceBetween(paris, eiffelTower, 5000);
        givenADistanceBetween(paris, louvre, 1000);

        assertEquals(List.of(new FeaturedDestinationSuggestion(paris, louvre, 1000), new FeaturedDestinationSuggestion(paris, eiffelTower, 5000)), recommendations.recommendationsFor(Set.of(paris)));
    }

    @Test
    void returns_recommendations_for_multi_location() {
        givenFeaturedDestinationsFor(paris, List.of(eiffelTower, louvre));
        givenADistanceBetween(paris, eiffelTower, 5000);
        givenADistanceBetween(paris, louvre, 1000);

        givenFeaturedDestinationsFor(alton, List.of(flowerFarm, watercressLine));
    ...
    @Test
    void deduplicates_using_smallest_distance() {
        givenFeaturedDestinationsFor(alton, List.of(flowerFarm, watercressLine));
        givenADistanceBetween(alton, flowerFarm, 5300);
        givenADistanceBetween(alton, watercressLine, 320);

        givenFeaturedDestinationsFor(froyle, List.of(flowerFarm, watercressLine));
    ...
    private void givenFeaturedDestinationsFor(Location location, List<FeaturedDestination> result) {
        when(featuredDestinations.findCloseTo(location)).thenReturn(result);
    }

    private void givenADistanceBetween(Location location, FeaturedDestination destination, int result) {
        when(distanceCalculator.distanceInMetersBetween(location, destination.getLocation())).thenReturn(result);
    }
}